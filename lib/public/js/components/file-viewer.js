...
const kFileViewerModeStorageKey = "alphaclawBrowseFileViewerMode";
...
export const FileViewer = ({ filePath = "" }) => {
  ...
  const handleSave = async () => {
    if (!canEditFile || saving || !isDirty) return;
    setSaving(true);
    setError("");
    try {
  ...
  return html`
    <div class="file-viewer">
      <div class="file-viewer-tabbar">
        ...
        `}
        <button
          class=${`file-viewer-save-btn ${isDirty ? "active" : ""}`}
          disabled=${saving || loading || !isDirty || !canEditFile}
          onclick=${handleSave}
        >
